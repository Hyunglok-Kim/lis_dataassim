...
!  28 Jan 2021: Hyunglk Kim: Updated for CYGNSS daily/subdaily/SMAP combiend DA
!
module CYGNSSsm_Mod
! !USES: 
  use ESMF
...
!-----------------------------------------------------------------------------
  public :: CYGNSSsm_setup
!-----------------------------------------------------------------------------
! !PUBLIC TYPES:
!-----------------------------------------------------------------------------
  public :: CYGNSSsm_struc
!EOP
  type, public:: CYGNSSsm_dec
  ...
  end type CYGNSSsm_dec
  
  type(CYGNSSsm_dec),allocatable :: CYGNSSsm_struc(:)
  
contains

!BOP
! 
! !ROUTINE: CYGNSSsm_setup
! \label{CYGNSSsm_setup}
! 
! !INTERFACE: 
  subroutine CYGNSSsm_setup(k, OBS_State, OBS_Pert_State)
! !USES: 
    use ESMF
    use LIS_coreMod
    use LIS_timeMgrMod
...
!   creation of data strctures required for handling CYGNSS 
!   soil moisture data. 
!  
!   The arguments are: 
!   \begin{description}
...
!EOP
    ...
    allocate(CYGNSSsm_struc(LIS_rc%nnest))
    ...
    call ESMF_ConfigFindLabel(LIS_config,"CYGNSS soil moisture data directory:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,rtsmopssmobsdir,&
            rc=status)
       call LIS_verify(status, 'CYGNSS soil moisture data directory: is missing')
    ...
    call ESMF_ConfigFindLabel(LIS_config,"CYGNSS soil moisture data designation:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,&
            CYGNSSsm_struc(n)%data_designation,&
            rc=status)
       call LIS_verify(status, 'CYGNSS soil moisture data designation: is missing')
    enddo

    if (CYGNSSsm_struc(n)%data_designation.eq."CYG_SMAP") then 
       call ESMF_ConfigFindLabel(LIS_config,"SMAP(NASA) soil moisture Composite Release ID:",&
            rc=status)
       do n=1,LIS_rc%nnest
          call ESMF_ConfigGetAttribute(LIS_config,&
               CYGNSSsm_struc(n)%release_number,&
               rc=status)
          call LIS_verify(status, 'SMAP(NASA) soil moisture Composite Release ID: is missing')
       enddo
    endif

    call ESMF_ConfigFindLabel(LIS_config,"CYGNSS soil moisture use scaled standard deviation model:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,CYGNSSsm_struc(n)%useSsdevScal,&
            rc=status)
       call LIS_verify(status, 'CYGNSS soil moisture use scaled standard deviation model: is missing')
       
    enddo

    call ESMF_ConfigFindLabel(LIS_config,"CYGNSS model CDF file:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,CYGNSSsm_struc(n)%modelcdffile,rc=status)
       call LIS_verify(status, 'CYGNSS model CDF file: not defined')
    enddo

    call ESMF_ConfigFindLabel(LIS_config,"CYGNSS observation CDF file:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,CYGNSSsm_struc(n)%obscdffile,rc=status)   
       call LIS_verify(status, 'CYGNSS observation CDF file: not defined')
    enddo
    
    call ESMF_ConfigFindLabel(LIS_config, "CYGNSS soil moisture number of bins in the CDF:", rc=status)
    do n=1, LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,CYGNSSsm_struc(n)%nbins, rc=status)
       call LIS_verify(status, "CYGNSS soil moisture number of bins in the CDF: not defined")
    enddo

   do n=1, LIS_rc%nnest
      CYGNSSsm_struc(n)%cdf_read_mon = .false.   

      call ESMF_ConfigFindLabel(LIS_config, "CYGNSS CDF read option:", rc=status)    ! 0: read CDF for all months/year 
                                                                                         ! 1: read CDF for current month
      call ESMF_ConfigGetAttribute(LIS_config, CYGNSSsm_struc(n)%cdf_read_opt, rc=status)
      call LIS_verify(status, "CYGNSS CDF read option: not defined")
   enddo
   ...
   do n=1,LIS_rc%nnest
    ...
    write(LIS_logunit,*)&
         '[INFO] read CYGNSS soil moisture data specifications'       

!----------------------------------------------------------------------------
!   Create the array containers that will contain the observations and
!   the perturbations. CYGNSS
...
! Set obs err to be uniform (will be rescaled later for each grid point). 
          ...
          CYGNSSsm_struc(n)%ssdev_inp = obs_pert%ssdev(1)
...
! initializing the perturbations to be zero 
    ...
    do n=1,LIS_rc%nnest
       !if(CYGNSSsm_struc(n)%data_designation.eq."SPL3SMP") then 
          CYGNSSsm_struc(n)%nc = 964
          CYGNSSsm_struc(n)%nr = 406
      ! elseif(CYGNSSsm_struc(n)%data_designation.eq."SPL2SMP_E") then
      !    CYGNSSsm_struc(n)%nc = 3856
      !    CYGNSSsm_struc(n)%nr = 1624
      ! endif

       allocate(CYGNSSsm_struc(n)%smobs(LIS_rc%obs_lnc(k),LIS_rc%obs_lnr(k)))
       allocate(CYGNSSsm_struc(n)%smtime(LIS_rc%obs_lnc(k),LIS_rc%obs_lnr(k)))
    ...
    do n=1,LIS_rc%nnest
       ...
       if(LIS_rc%dascaloption(k).eq."CDF matching") then 
          ...
          call LIS_getCDFattributes(k,CYGNSSsm_struc(n)%modelcdffile,&
               CYGNSSsm_struc(n)%ntimes,ngrid)                 
         
          if (CYGNSSsm_struc(n)%cdf_read_opt.eq.0) then    
             allocate(CYGNSSsm_struc(n)%model_xrange(&
                  LIS_rc%obs_ngrid(k), CYGNSSsm_struc(n)%ntimes, &
                  CYGNSSsm_struc(n)%nbins))
             allocate(CYGNSSsm_struc(n)%obs_xrange(&
                  LIS_rc%obs_ngrid(k), CYGNSSsm_struc(n)%ntimes, &
                  CYGNSSsm_struc(n)%nbins))
             allocate(CYGNSSsm_struc(n)%model_cdf(&
                  LIS_rc%obs_ngrid(k), CYGNSSsm_struc(n)%ntimes, &
                  CYGNSSsm_struc(n)%nbins))
             allocate(CYGNSSsm_struc(n)%obs_cdf(&
                  LIS_rc%obs_ngrid(k), CYGNSSsm_struc(n)%ntimes, &
                  CYGNSSsm_struc(n)%nbins))
             allocate(CYGNSSsm_struc(n)%model_mu(LIS_rc%obs_ngrid(k),&
                  CYGNSSsm_struc(n)%ntimes))
             allocate(CYGNSSsm_struc(n)%model_sigma(LIS_rc%obs_ngrid(k),&
                  CYGNSSsm_struc(n)%ntimes))
             allocate(CYGNSSsm_struc(n)%obs_mu(LIS_rc%obs_ngrid(k),&
                  CYGNSSsm_struc(n)%ntimes))
             allocate(CYGNSSsm_struc(n)%obs_sigma(LIS_rc%obs_ngrid(k),&
                  CYGNSSsm_struc(n)%ntimes))
          else  
             allocate(CYGNSSsm_struc(n)%model_xrange(&
             ...
             allocate(CYGNSSsm_struc(n)%obs_cdf(&
                  LIS_rc%obs_ngrid(k), 1, &
                  CYGNSSsm_struc(n)%nbins))
             allocate(CYGNSSsm_struc(n)%model_mu(LIS_rc%obs_ngrid(k),1))
             allocate(CYGNSSsm_struc(n)%model_sigma(LIS_rc%obs_ngrid(k),1))
             allocate(CYGNSSsm_struc(n)%obs_mu(LIS_rc%obs_ngrid(k),1))
             allocate(CYGNSSsm_struc(n)%obs_sigma(LIS_rc%obs_ngrid(k),1)) 
          endif 
!----------------------------------------------------------------------------
          ...
          if(CYGNSSsm_struc(n)%useSsdevScal.eq.1) then 
             ...
             do t=1,LIS_rc%obs_ngrid(k)
                if(CYGNSSsm_struc(n)%obs_sigma(t,jj).gt.0) then 
                   ...
                   print*, ssdev(t), CYGNSSsm_struc(n)%model_sigma(t,jj),&
...
!          stop
    ...
    do n=1,LIS_rc%nnest
       !if(CYGNSSsm_struc(n)%data_designation.eq."SPL3SMP".or.&
       !     CYGNSSsm_struc(n)%data_designation.eq."SPL2SMP") then
          gridDesci = 0
          gridDesci(1) = 9
          gridDesci(2) = 964
          gridDesci(3) = 406
       ...
       !elseif(CYGNSSsm_struc(n)%data_designation.eq."SPL3SMP_E".or.&
       !     CYGNSSsm_struc(n)%data_designation.eq."SPL2SMP_E") then
       !   gridDesci = 0
       !   gridDesci(1) = 9
       !   gridDesci(2) = 3856
       !   gridDesci(3) = 1624
       ...
       allocate(CYGNSSsm_struc(n)%n11(LIS_rc%obs_lnc(k)*&
            LIS_rc%obs_lnr(k)))
       allocate(CYGNSSsm_struc(n)%rlat(&
            LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
       allocate(CYGNSSsm_struc(n)%rlon(&
            LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
       
       call neighbor_interp_input_withgrid(gridDesci,&
            LIS_rc%obs_gridDesc(k,:),&
            LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k),&
            CYGNSSsm_struc(n)%rlat, &
            CYGNSSsm_struc(n)%rlon, &
            CYGNSSsm_struc(n)%n11)

       if(CYGNSSsm_struc(n)%data_designation.eq."CYG_SMAP") then
          call LIS_registerAlarm("CYGNSS read alarm",&
               3600.0, 3600.0)
       elseif(CYGNSSsm_struc(n)%data_designation.eq."CYG_SubDaily") then
          call LIS_registerAlarm("CYGNSS read alarm",&
               10800.0, 10800.0)
       else
          call LIS_registerAlarm("CYGNSS read alarm",&
       ...
       CYGNSSsm_struc(n)%startMode = .true. 
    ...
    enddo
  end subroutine CYGNSSsm_setup
end module CYGNSSsm_Mod
