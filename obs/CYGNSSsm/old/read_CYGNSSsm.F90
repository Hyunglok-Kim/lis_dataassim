...
!EOP
   ...
   write(LIS_logunit,*) 'time step',LIS_rc%ts
   
   write(LIS_logunit,*) 'alarmCheck ',alarmCheck

   write(LIS_logunit,*) 'startMode',CYGNSSsm_struc(n)%startMode
   if (alarmCheck .or. CYGNSSsm_struc(n)%startMode) then
      CYGNSSsm_struc(n)%startMode = .false.
      if ( (CYGNSSsm_struc(n)%data_designation.eq."SPL2SMP_E") .or. &
           (CYGNSSsm_struc(n)%data_designation.eq."CYG_SubDaily") ) then
        
         write(LIS_logunit,*) 'here' 
         ...
         if(LIS_masterproc) then
            list_files = "ls "//trim((smobsdir))//&
                 ...
                 "/SMAP_L2_SM_P_*_*_"//trim(yyyymmdd)//"T"//trim(hh)&
         ...
         do while(ierr.eq.0)
            ...
            call read_CYGNSS_SubDailysm_data(n,k,smap_filename(i),&
...
!
      ...
      elseif (CYGNSSsm_struc(n)%data_designation .eq. "SPL3SMP_E") then
!---------------------------------------------------------------------------
...
!---------------------------------------------------------------------------
         ...
         write (mm, '(i2.2)') LIS_rc%mo
         write (dd, '(i2.2)') LIS_rc%da
         write (CRID, '(a)') CYGNSSsm_struc(n)%release_number
...
! This assumes that the 'ls command' will list the files in that order.
         ...
         do while (ierr .eq. 0)
            ...
            call read_CYGNSS_E_data(n, k, 'D', fname, smobs_D)

            write (LIS_logunit, *) '[INFO] Reading ascending pass ', trim(fname)
            call read_CYGNSS_E_data(n, k, 'A', fname, smobs_A)
         enddo
...
!-------------------------------------------------------------------------
                  if (smobs_A(c + (r - 1)*LIS_rc%obs_lnc(k)) .ne. -9999.0 .and. &
                      CYGNSSsm_struc(n)%smobs(c, r) .eq. -9999.0) then
                     CYGNSSsm_struc(n)%smobs(c, r) = &
      ...
      elseif (CYGNSSsm_struc(n)%data_designation .eq. "SPL3SMP") then
!-------------------------------------------------------------------------
...
! This assumes that the 'ls command' will list the files in that order.
         ...
         do while (ierr .eq. 0)
            ...
            write (LIS_logunit, *) '[INFO] Reading descending pass ', trim(fname)
            call read_CYGNSS_data(n, k, 'D', fname, smobs_D)

            write (LIS_logunit, *) '[INFO] Reading ascending pass ', trim(fname)
            call read_CYGNSS_data(n, k, 'A', fname, smobs_A)
         enddo
...
!BOP
! 
! !ROUTINE: read_CYGNSS_E_data
! \label{read_CYGNSS_E_data}
!
! !INTERFACE:
subroutine read_CYGNSS_E_data(n, k, pass, fname, smobs_ip)
! 
! !USES:   
...
!  \begin{description}
!  \item[n]            index of the nest
!  \item[fname]        name of the RTCYGNSS AMSR-E file
...
! MN 
  ...
  allocate(sm_qa(CYGNSSsm_struc(n)%nc, CYGNSSsm_struc(n)%nr))
  allocate(vwc_field(CYGNSSsm_struc(n)%nc, CYGNSSsm_struc(n)%nr))
  allocate(dims(2))
  ...
  if(pass.eq.'D') then 
     ...
     call LIS_verify(status, 'Error opening SM QA field in CYGNSS file')
     
     call h5dread_f(sm_qa_id_D, H5T_NATIVE_INTEGER,sm_qa,dims,status, &
          memspace, dataspace)
     call LIS_verify(status, 'Error extracting SM QA field from CYGNSSfile')
...
! MN get the vegetation water contnent 
     ...
     call LIS_verify(status, 'Error opening Veg water content field in CYGNSS file')

     call h5dread_f(vwc_field_id_D, H5T_NATIVE_REAL,vwc_field,dims,status, &
          memspace, dataspace)
     call LIS_verify(status, 'Error extracting Veg water content (AM) field from CYGNSSfile')
...
end subroutine read_CYGNSS_E_data

! MN: the data structure in both 36 km and 9 km products is the same therefore  
!         read_CYGNSS_E_data is similar to read_CYGNSS_data
...
!
! !INTERFACE:
subroutine read_CYGNSS_data(n, k, pass, fname, smobs_ip)
! 
! !USES:   
...
end subroutine read_CYGNSS_data
