...
!  25 Jan 2021: Hyunglok Kim: Modifeid for reading CYGNSS
!  daily/subdaily/SMAP_combined SM data  
!
module CYGNSSsm_Mod
! !USES: 
  use ESMF
...
! initializing the perturbations to be zero 
    ...
    enddo
    write(LIS_logunit,*) &
         '[INFO] Created the States to hold the CYGNSS observations data'
    do n=1,LIS_rc%nnest
       if(CYGNSSsm_struc(n)%data_designation.eq."CYG_Daily".or.&
          CYGNSSsm_struc(n)%data_designation.eq."CYG_SubDaily".or.&
          CYGNSSsm_struc(n)%data_designation.eq."CYG_SMAP") then        
          CYGNSSsm_struc(n)%nc = 964
          CYGNSSsm_struc(n)%nr = 406
       elseif(CYGNSSsm_struc(n)%data_designation.eq."CYG_UCAR") then
          CYGNSSsm_struc(n)%nc = 964
          CYGNSSsm_struc(n)%nr = 406
       endif
...
!          stop
          ...
          do r=1,LIS_rc%obs_lnr(k)
             ...
             enddo
          enddo
          print*, 'CYGNSS ',LIS_rc%obs_lnc(k),LIS_rc%obs_lnr(k)
          open(100,file='cygnss_obs_err.bin',form='unformatted')
          write(100) obserr
          close(100)
          stop
          deallocate(obserr)
...
!          stop
    ...
    do n=1,LIS_rc%nnest
       ...
       elseif (CYGNSSsm_struc(n)%data_designation.eq."CYG_UCAR") then!update needs
          gridDesci = 0
          gridDesci(1) = 9
          gridDesci(2) = 964 
          gridDesci(3) = 406 
          gridDesci(9) = 4 !M09 grid
          gridDesci(20) = 64
          gridDesci(10) = 0.36 
          gridDesci(11) = 1 !for the global switch
       endif
